      ...
      <section className="px-6 pb-10 mt-10 md:px-14 lg:px-24">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
          { courses && Array.isArray(courses) && courses.map((course) => {
            return <Link href={`/course/${course.id}`} key={course.id}><CourseCardMy image={course.image_url} title={course.title} description={course.description} categories={course.categories} id={course.id} author={course.author_id} /></Link>
          })}
        </div>
      </section>
    </Layout>
